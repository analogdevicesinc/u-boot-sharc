...
class Proc():
    ...
    def __init__(self, cmd, logger=None, ubuntu_package=None, url=None, max_bytes_to_log=1000):
        '''
...
#            log.debug('stopping thread')
    ...
    def _wait4process(self):
        ...
        if self.popen:
            if not self.logger:    
                ...
                while True:
                    line=remove_ending_lf(self.popen.stdout.readline())
                    if line == '' and self.popen.poll() != None:
                        break

                    self.logger("> %s" % line.rstrip())
                ...
                # close named pipes
                try:
                    if self.popen.stdout:self.popen.stdout.close()
                    if self.popen.stdin:self.popen.stdin.close()
                    if self.popen.stderr:self.popen.stderr.close()
                except: pass
    ...
    def sendstop(self):
        ...
        if self.popen:
            if self.is_alive():
                log.debug('process is active -> terminate process tree.')

                #os.kill(self.popen.pid, signal.SIGKILL)
                try:
                    #for windows os only
                    if platform.system()=='Windows':
                        os.system("taskkill /pid "+str(self.pid)+" /f /t")
                    #TODO:for linux os only
                    #elif platform.system()=='Linux':
                        #TODO
                    self.return_code = self.EP_RET_TIMEOUT
                except OSError as e:
